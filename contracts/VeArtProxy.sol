...
contract VeArtProxy is IVeArtProxy {
    ...
    uint128 private constant PI          = 3141592653589793238;
    uint128 private constant TWO_PI      = 2 * PI;
    uint32 private constant DASH        = 50;
    uint32 private constant DASH_HALF   = 25;
    ...
    function tokenURI (uint256 _tokenId) external view returns (string memory) {
        ...
        if(cfg._lockedEnd == 0) {
            ...
            (uint64 year, uint64 month, uint64 day) = BokkyPooBahsDateTimeLibrary.timestampToDate(uint64(uint256(cfg._lockedEnd)));
            date = bytes32(abi.encodePacked('"', toString(year), '-', toString(month), '-', toString(day), '"'));
        }

        return string(abi.encodePacked("data:application/json;base64,", jsonEncode(
            cfg._tokenId,
            makeAttributes(date, cfg._balanceOf, cfg._lockedAmount),
            Base64.encode(
                abi.encodePacked(
    ...
    function makeAttributes(bytes32 date, int256 balanceOf, int256 lockedAmount) private pure returns (bytes memory) {
        return abi.encodePacked(
            '{"trait_type": "Unlock Date","value": ',
            date,
            '},{"trait_type": "Voting Power","value": ',
            toString(balanceOf / 1e18),
            '},{"trait_type": "Locked AERO","value": ',
            toString(lockedAmount / 1e18),
            '},{"display_type": "number","trait_type": "Number of Digits","value": ',
            toString(numBalanceDigits(uint256(balanceOf))),
            '}'
        );
    }

    function jsonEncode(int256 tokenId, bytes memory attributes, string memory output) private pure returns (string memory) {
        return Base64.encode(
		    abi.encodePacked(
                '{"name": "lock #', toString(tokenId),
                '","background_color": "121a26","description": "Aerodrome Finance is a next-generation AMM that combines the best of Curve, Convex and Uniswap, designed to serve as the Base central liquidity hub. Aerodrome NFTs vote on token emissions and receive bribes and fees generated by the protocol.","image_data": "data:image/svg+xml;base64,',
                output,
                '","attributes": [',
                attributes,
                ']}'
    ...
    /// @inheritdoc IVeArtProxy
    function generateConfig (uint256 _tokenId) public view returns (Config memory) {
        IVotingEscrow.LockedBalance memory _locked = ve.locked(_tokenId);
        return Config({
            _tokenId: int256(_tokenId),
            _balanceOf: int256(ve.balanceOfNFTAt(_tokenId, block.timestamp)),
            _lockedEnd: int256(_locked.end),
            _lockedAmount: int256(_locked.amount),
            shape: seedGen(_tokenId) % 8,
            palette: getPalette(_tokenId),
            maxLines: getLineCount(ve.balanceOfNFTAt(_tokenId, block.timestamp)),
            dash: 0,
            seed1: seedGen(_tokenId),
            seed2: seedGen(_tokenId * 1e18),
            seed3: seedGen(_tokenId * 2e18)
        });
    }

    /// @dev Generates characteristics for each line in the line art.
    function generateLineConfig (Config memory cfg, int128 l) internal view returns (lineConfig memory linecfg) {
        linecfg.color = palettes[cfg.palette][uint256(keccak256(abi.encodePacked((l + 20) * (cfg._lockedEnd + cfg._tokenId)))) % 5];
        linecfg.stroke = uint32(uint256(keccak256(abi.encodePacked((l + 1) * (cfg._lockedEnd + cfg._tokenId)))) % 5);
        linecfg.offset = uint32(uint256(keccak256(abi.encodePacked((l + 1) * (cfg._lockedEnd + cfg._tokenId)))) % 50 / 2 * 2 * 5); // ensure value is even
        linecfg.offsetHalf = linecfg.offset / 2 * 5;
        linecfg.offsetDashSum = linecfg.offset + DASH + linecfg.offsetHalf + DASH_HALF;
        if ((uint128(cfg.seed2) / (1 + uint128(l))) % 6 != 0) {
            linecfg.pathLength = linecfg.offsetDashSum * (10 + (uint128(cfg.seed1 * cfg.seed3) / (1 + uint128(l) * uint128(l))) % 15);
        }
    }
       
    /// @dev Selects and draws line art shape.
    function generateShape (Config memory cfg) internal view returns (bytes memory) {
        if (cfg.shape == 0) {
            return drawCircles(cfg);
        }
        else if (cfg.shape == 1) {
            return drawTwoStripes(cfg);
        }
        else if (cfg.shape == 2) {
            return drawInterlockingCircles(cfg);
        }
        else if (cfg.shape == 3) {
            return drawCorners(cfg);
        }
        else if (cfg.shape == 4) {
            return drawCurves(cfg);
        }
        else if (cfg.shape == 5) {
            return drawSpiral(cfg);
        }
        else if (cfg.shape == 6) {
            return drawExplosion(cfg);
        }
        else {
            return drawWormhole(cfg);
        }
    }
    ...
    function seedGen (uint256 _tokenId) internal pure returns (int32 seed) {
        seed = 1 + int32(int256(uint256(keccak256(abi.encodePacked(_tokenId))) % 999));
    }
    ...
    function twoStripes(Config memory cfg, int256 l) public pure returns (Point[100] memory Line) {
        TwoStripesBuffer memory b;
        b.k = ((l % 2) * ((200 + cfg.seed3) + (l * 1250 / cfg.maxLines)) + ((l + 1) % 2) * ((2200 + cfg.seed2) + (l * 1250 / cfg.maxLines)));
        b.i1 = cfg.seed1 % 2;
        b.i2 = (cfg.seed1 + 1) % 2;
        b.o1 = b.i1 * b.k;
        b.o2 = b.i2 * b.k;
    ...
    /// @inheritdoc IVeArtProxy
    function circles(Config memory cfg, int64 l) public pure returns (Point[100] memory Line) {
        CirclesBuffer memory b;
        b.baseX = 500 + (int64(cfg.seed1) % 100 * 30);
        b.baseY = 500 + (int64(cfg.seed2) % 100 * 30);
        b.k  = (int64(cfg.seed3) % 250) + 250 + 100 * (1 + l);
        b.i = (l % 2) * 2 - 1;

        for (uint256 p = 0; p < 100;) {
            uint256 angle = 1e18 * TWO_PI * p / 99;
            Line[p] = Point({
                x: b.baseX +     b.k * Trig.sin(angle) / 1e18,
                y: b.baseY + b.i * b.k * Trig.cos(angle) / 1e18
            });
            unchecked { ++p; }
        }        
    }
    ...
    /// @inheritdoc IVeArtProxy
   function corners(Config memory cfg, int32 l) public pure returns (Point[100] memory Line) {
        CornersBuffer memory b;
        b.degrees1 = 360 * int32(cfg.seed1) / 1000;
        b.degrees2 = 360 * (int32(cfg.seed1) + 500) / 1000;
        b.baseX = 2000 + int32(((l % 2) * 1200 * Trig.dcos(b.degrees1) / 1e6) + (((l + 1) % 2) * (1200 * Trig.dcos(b.degrees2)) / 1e6));
        b.baseY = 2000 + int32(((l % 2) * 1200 * Trig.dsin(b.degrees1) / 1e6) + (((l + 1) % 2) * (1200 * Trig.dsin(b.degrees2)) / 1e6));
        b.k = 100 + (1 + l) * 4000 / int32(cfg.maxLines) / 4;

        for (int32 p; p < 100;) {
            b.angle3 = 360 * l / int32(cfg.maxLines) + (360 * p / 99);
            Line[uint256(int256(p))] = Point({
                x: b.baseX +                     b.k * Trig.dcos(b.angle3) / 1e6,
                y: b.baseY + ((l % 2) * 2 - 1) * b.k * Trig.dsin(b.angle3) / 1e6
            });
            unchecked { ++p; }
        }
    }
    ...
    function drawExplosion (Config memory cfg) internal view returns (bytes memory shape) { 
        for (int128 l = 0; l < cfg.maxLines; l++) {
    ...
    function curveToSVG (int128 l, Config memory cfg, Point[100] memory Line) internal view returns (bytes memory SVGLine) {
